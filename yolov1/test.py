    ...
    if dataset == 'voc':
        for i, box in enumerate(bboxes):
            ...
            if scores[i] > thresh:
                ...
                cv2.rectangle(img, (int(xmin), int(abs(ymin) - 20)),
                              (int(xmax), int(ymin)),
                              class_colors[int(cls_indx)], -1)
                mess = '%s' % (class_names[int(cls_indx)])
                cv2.putText(img, mess, (int(xmin), int(ymin - 5)),
...
def test(net,
    ...
    for index in range(num_images):
        ...
        x = torch.from_numpy(transform(img)[0][:, :,
                                               (2, 1, 0)]).permute(2, 0, 1)
        ...
        # 可视化检测结果
        img_processed = vis(img, bboxes, scores, cls_inds, thresh,
                            class_colors, class_names, class_indexs, dataset)
